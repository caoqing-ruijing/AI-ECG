...
class DenseNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.features = nn.Sequential(OrderedDict([
            ("conv0", nn.Conv2d(1, num_init_features, kernel_size=(3, 7), stride=(1, 2), padding=(1, 3), bias=False)),
            ("norm0", nn.BatchNorm2d(num_init_features)),
            ("relu0", nn.ReLU(inplace=True)),
            ("pool0", nn.MaxPool2d(kernel_size=(1, 3), stride=(1, 2), padding=(0, 1))),
        ]))
...
if __name__ == "__main__":
    # input = torch.rand(32, 1, 1, 4000)
    input = torch.rand(1, 1, 12, 1250)
    model = densenet121()
    output = model(input)
    print(output.shape)
